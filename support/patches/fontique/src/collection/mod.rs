...
use super::{
    Attributes, Blob, FontStyle, FontWeight, FontWidth, GenericFamily, Language, Script,
    backend::SystemFonts,
...
impl Collection {
    ...
    /// Returns explicit fallback families, or on Windows one unscaled system
    /// fallback family selected for the supplied complete text range.
    pub fn fallback_families_for_text(
        &mut self,
        key: impl Into<FallbackKey>,
        text: &str,
        ...
        self.inner
            .fallback_families_for_text(key.into(), text, attributes)
            .into_iter()
    }
...
impl Inner {
    ...
    fn fallback_families_for_text(
        ...
        self.sync_shared();
        if let Some(families) = self.data.fallbacks.get(key) {
            return families.to_vec();
        }
        #[cfg(all(target_os = "windows", feature = "system"))]
        if let Some(system) = self.system.as_ref() {
            let mut system = system.fonts.lock().unwrap();
            if let Some(family) = system.fallback_for_text(key, text, attributes) {
                return vec![family];
            }
        }
        #[cfg(not(all(target_os = "windows", feature = "system")))]
        let _ = (text, attributes);
        self.fallback_families(key).collect()
    }
